...
const {DEFAULT_LEVEL, LEVELS, COLORS} = require('../constants/logger.js');

/* custom formats */

// winston.format.padLevels() seems to be broken, so this is a custom version
const customPadLevels = winston.format(info => {
	const maxLength = Math.max(...Object.keys(LEVELS).map(e => e.length));
...
const validLoggingLevel = Object.keys(LEVELS).includes(process.env.LOGGING_LEVEL);

const logger = winston.loggers.add('default', {
	level: validLoggingLevel ? process.env.LOGGING_LEVEL : DEFAULT_LEVEL,
	levels: LEVELS,
...
if (!validLoggingLevel) {
	logger.error(`Invalid logging level, defaulting to "${DEFAULT_LEVEL}".`);
}
